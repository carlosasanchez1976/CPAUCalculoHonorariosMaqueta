/**
 * Algoritmo de cálculo de honorarios CPAU - Tipo Básico
 * 
...
/**
 * Obtiene metadata del cálculo (rango, valorK, etc.)
 * @param {Object} formData - Datos del formulario completo
 * @param {number} valorK - Valor K utilizado
 * @returns {Object} Metadata del cálculo
 */
export function obtenerMetadataCalculo(formData, valorK = VALOR_K_DEFAULT) {
  const rangoCostoDeObra = formData.valorObra / valorK;
  const rango = determinarRango(rangoCostoDeObra);
  const nombreRango = NOMBRES_RANGO[rango];
